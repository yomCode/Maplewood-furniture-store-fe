          ...
          <div className="col-md-8 bg-white drop-shadow-md rounded-md pb-5 divide-y">
            <div className="divide-y">
              <div className="border-gray-300 border-b-1 py-3">
                ...
                <div className="mt-3">
                  <Link
                    to="/open-orders"
                    className="border-r border-black pr-3 hover:text-red-600"
                  >
                    OPEN ORDERS
                  </Link>
                  <Link to="/closed-orders" className="ml-3">
                    CLOSED ORDERS{" "}
                  </Link>
                </div>
              </div>
              ...
              {closedOrders.length === 0 && (
                <div className="mx-2">
                  ...
                  <div className="flex justify-center align-middle py-2 text-xl text-center">
                    <h3>
                      All your closed orders will be saved here for you to
                      access their state anytime.
                    </h3>
                  </div>
                  <div className="flex justify-center align-middle py-3  text-xl">
                    <Link
                      to="/shop"
                      className="bg-[#917307] text-white px-3 py-3 rounded-md drop-shadow-md hover:text-xl hover:bg-[#796006]"
                    >
                      CONTINUE SHOPPING
                    </Link>
                  </div>
            ...
            {closedOrders !== null &&
              closedOrders.map((order, index) => {
                ...
                const dateCreatedFormatted = dateCreated.toLocaleDateString(
                  "en-US",
                  dateOptions
                );
                const dateUpdatedFormatted = dateUpdated.toLocaleDateString(
                  "en-US",
                  dateOptions
                );

                const timeOptions = { hour: "numeric", minute: "2-digit" };
                const timeCreatedFormatted = dateCreated.toLocaleTimeString(
                  "en-US",
                  timeOptions
                );
                const timeUpdatedFormatted = dateUpdated.toLocaleTimeString(
                  "en-US",
                  timeOptions
                );
                return (
                  <div className="row border-gray-300 border-b" key={index}>
                    ...
                    <div>
                      ...
                      <p>
                        Transaction Id: {order.transaction.reference}{" "}
                    ...
                    <div className="col-md-6">
                      <CheckoutCard
                        title={`1. Your Order(${order.items.length} items)`}
                      >
                        <div className="divide-y">
                    ...
                    <div className="col-md-6">
                      <CheckoutCard title="2. PICKUP CENTER DETAILS">
                        <ul className="text-lg mt-0 px-3">
                          ...
                          <li className="text-lg font-medium">
                            - Address: {order.pickupCenter.address}
                          </li>
                          <li className="text-lg font-medium">
                            - State: {order.pickupCenter.state.name}
                          </li>
                          <li className="text-lg font-medium">
                            - Phone-number: {order.pickupCenter.phone}
                          </li>
                          <li className="text-lg font-medium">
                            - Email: {order.pickupCenter.email}
                          </li>
                        </ul>
                      </CheckoutCard>
                    </div>
