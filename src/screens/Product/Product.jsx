...
const Product = ({
  ...
  const onClick = ({ key }) => {
    if (!showFavorites) {
      switch (key) {
        ...
        case "2":
          setProducts([
            ...products.sort((a, b) => (a.imageUrl > b.imageUrl ? 1 : -1)),
          ]);
          return;
        case "3":
      ...
      switch (key) {
        ...
        case "2":
          setFavorites([
            ...favorites.sort((a, b) => (a.imageUrl > b.imageUrl ? 1 : -1)),
          ]);
          return;
        case "3":
  ...
  useEffect(() => {
    showFavorites
      ...
      : setProductUrl(
          isId ? `${productUrlProp}/${params.id}/paginated-all` : productUrlProp
        );
  }, [
    productUrlProp,
    setProductUrl,
      ...
      <div className="products-info">
        {showFavorites ? (
          favNumOfElements < pageElementSize ? (
            <p>
              Showing {favPageNumber * favPageElementSize + 1} -{" "}
              {favTotalElements} of {favTotalElements}
            </p>
          ) : (
            <p>
              Showing {favNumOfElements * favPageNumber + 1} -{" "}
              {favNumOfElements * (favPageNumber + 1)}
              of {favTotalElements} results
            </p>
          )
