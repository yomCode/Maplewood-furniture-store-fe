...
const DataProvider = ({ children }) => {
  ...
  const [closedOrdersPageElementSize, setClosedOrdersPageElementSize] =
    useState(0);
  ...
  const ForgottenConfig = async (formData) => {
    try {
      ...
      await apiPost("auth/forgot-password-request", registerData).then(
        (res) => {
  ...
  const FetchTrx = useCallback(() => {
    try {
      apiGetAuthorization(
        `customer/wallet/transactions?pageNo=${pageNumber}`
      ).then((res) => {
        const data = res.data;
  ...
  const GetAllOpenOrders = useCallback(() => {
    try {
      apiGetAuthorization(
        `customer/order/pickup-status?status=YET_TO_BE_PICKED_UP&pageNo=${openOrdersPageNumber}`
      ).then((res) => {
        const data = res.data;
  ...
  const GetAllClosedOrders = useCallback(() => {
    try {
      apiGetAuthorization(
        `customer/order/pickup-status?status=PICKED_UP&pageNo=${closedOrdersPageNumber}`
      ).then((res) => {
        const data = res.data;
  ...
  const OrderConfig = async (formData) => {
    try {
      ...
      await apiPostAuthorization(`customer/order/new`, saveOrderData).then(
        (res) => {
          Swal.fire({
            title: res.data,
