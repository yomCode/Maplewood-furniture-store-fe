...
export const CategoryProvider = ({ children }) => {
  ...
  const [categories, setCategories] = useState([
    ...
    },
    {
      ...
      name: "DRAWER",
      imageUrl:
        "https://templatekit.jegtheme.com/funiture/wp-content/uploads/sites/18/2020/11/Image-RBSJBF@2x-300x300.jpg",
    },
    {
      id: 3,
      name: "TABLE",
    ...
    apiPostAuthorization(`subcategory/admin/new/${singleCategory.id}`, data)
      .then((res) => {
        ...
        successNotification(
          ...
          `${data.name} was added to the system.`
        );
        getCategories();
      })
      .catch((err) => {
  ...
  const createNewPickupCenter = (setSubmitting, onClose, newPickupCenter) => {
    ...
    apiPostAuthorization("pickup/new", newPickupCenter)
      .then((res) => {
        ...
        onClose();
        successNotification(
          "Pickup Center Created",
          `${newPickupCenter.name} was added to the system.`
        );
        getPickupCenters();
        setSubmitting(false);
      })
  ...
  const createNewState = (setSubmitting, onClose, newState) => {
    ...
    apiPostAuthorization("state/admin/create_state", newState)
      .then((res) => {
        ...
        successNotification(
          "Successful",
          `${newState.nameOfState} was added to the system.`
        );
        getAllStates();
      })
      .catch((err) => {
